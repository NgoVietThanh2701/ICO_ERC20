...
describe("---LCKCrowdSale", function () {
   ...
   beforeEach(async () => {
      ...
      const LCKCROWDSALE = await ethers.getContractFactory("LCKCrowdSale", owner);
      LCKCrowdSale = await LCKCROWDSALE.deploy(1000, owner.address, token.address);
   })

   // positive testing
   it("Buy token by BNB", async () => {
      await token.transfer(LCKCrowdSale.address, parseEther(50000));
   ...
   it("Buy token by BNB with new rate bnb", async () => {
      await LCKCrowdSale.connect(owner).setBNBRate(100);
      await token.transfer(LCKCrowdSale.address, parseEther(50000));
      await LCKCrowdSale.connect(alice).buyTokenByBNB({ value: parseEther(5) });
      expect(await token.balanceOf(alice.address)).equal(parseEther(500));
      expect(await token.balanceOf(LCKCrowdSale.address)).equal(parseEther(49500));
   });
   // negative testing
   it("Should not buy BNB, Insufficient account balance(address(this))", async () => {
      await expect(LCKCrowdSale.connect(alice).buyTokenByBNB({ value: parseEther(5) })).revertedWith('Insufficient account balance in address(this)');
   })
   it("Should not buy BNB, Insufficient account balance(address(this))", async () => {
      await expect(LCKCrowdSale.connect(alice).setBNBRate(100)).revertedWith("Ownable: caller is not the owner");
   })
})